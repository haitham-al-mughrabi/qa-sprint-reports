...
def create_report():
    ...
    try:
        ...
        new_report = Report(
            ...
            # Evaluation Data
            involvementScore=int(data.get('involvementScore') or 0) if data.get('involvementScore', '').strip() else 0,
            involvementReason=data.get('involvementReason', ''),
            requirementsQualityScore=int(data.get('requirementsQualityScore') or 0) if data.get('requirementsQualityScore', '').strip() else 0,
            requirementsQualityReason=data.get('requirementsQualityReason', ''),
            qaPlanReviewScore=int(data.get('qaPlanReviewScore') or 0) if data.get('qaPlanReviewScore', '').strip() else 0,
            qaPlanReviewReason=data.get('qaPlanReviewReason', ''),
            uxScore=int(data.get('uxScore') or 0) if data.get('uxScore', '').strip() else 0,
            uxReason=data.get('uxReason', ''),
            cooperationScore=int(data.get('cooperationScore') or 0) if data.get('cooperationScore', '').strip() else 0,
            cooperationReason=data.get('cooperationReason', ''),
            criticalBugsScore=int(data.get('criticalBugsScore') or 0) if data.get('criticalBugsScore', '').strip() else 0,
            criticalBugsReason=data.get('criticalBugsReason', ''),
            highBugsScore=int(data.get('highBugsScore') or 0) if data.get('highBugsScore', '').strip() else 0,
            highBugsReason=data.get('highBugsReason', ''),
            mediumBugsScore=int(data.get('mediumBugsScore') or 0) if data.get('mediumBugsScore', '').strip() else 0,
            mediumBugsReason=data.get('mediumBugsReason', ''),
            lowBugsScore=int(data.get('lowBugsScore') or 0) if data.get('lowBugsScore', '').strip() else 0,
...
def get_cached_dashboard_stats():
    """Get dashboard statistics with detailed breakdown - simplified approach"""
    from sqlalchemy import func
    try:
        # Get overall stats directly from reports
        total_reports = db.session.query(func.count(Report.id)).scalar()
        completed_reports = db.session.query(func.count(Report.id)).filter(Report.testingStatus == 'passed').scalar()
        in_progress_reports = db.session.query(func.count(Report.id)).filter(Report.testingStatus == 'passed-with-issues').scalar()
        pending_reports = total_reports - completed_reports - in_progress_reports
