...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    if (reportId) {
        ...
        if (report) {
            ...
            document.getElementById('formTitle').textContent = 'Edit Sprint Report';
        } else {
            showToast('Report not found for editing.', 'error');
            editingReportId = null; // Reset if not found
        }
    } else {
        document.getElementById('formTitle').textContent = 'Create Sprint Report';
...
function updateReportTitle() {
    ...
    if (customName) {
        ...
        if (editingReportId) {
            formTitle.textContent = 'Edit Sprint Report';
        } else {
            formTitle.textContent = 'Create Sprint Report';
        }
    }
}
